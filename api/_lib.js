'use strict';
// Shared scraping logic — imported by tours.js and cron.js
...
async function fetchHT({ nightsFrom=7, nightsTo=9, country='vietnam', adults=2 }={}) {
  ...
  const uuid=init.id;
  if(!uuid) throw new Error('ht.kz: no search UUID');
...
async function fetchHappyTravel({ nightsFrom=7, nightsTo=9, country='vietnam', adults=2 }={}) {
  ...
  if(!requestId) throw new Error('Tourvisor: no requestid');
  ...
  if(!data) throw new Error('Tourvisor: no results');
...
async function fetchAllCountry({ nightsFrom=7, nightsTo=9, country='vietnam', adults=2 }={}) {
  const t0=Date.now();
  ...
  const all=[
    ...[...ht].sort((a,b)=>a.price-b.price).slice(0,20),
    ...[...happy].sort((a,b)=>a.price-b.price).slice(0,20),
...
module.exports = { fetchAllCountry };
